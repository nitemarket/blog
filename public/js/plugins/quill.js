          ...
          if (fileInput == null) {
            ...
            fileInput.addEventListener('change', function () {
              if (fileInput.files != null && fileInput.files[0] != null) {
				  			_this3.quill.options.imageHandler(fileInput.files[0], (result) => {
                  var range = _this3.quill.getSelection(true);
                  _this3.quill.updateContents(new _quillDelta2.default().retain(range.index).delete(range.length).insert({ image: result }), _emitter2.default.sources.USER);
                  fileInput.value = "";
								});
              }
            });
